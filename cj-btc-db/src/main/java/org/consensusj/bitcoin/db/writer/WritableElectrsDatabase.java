...
import org.consensusj.bitcoin.db.reader.ElectrsDatabase;

/**
 * Writable interface to an electrs RocksDB database.
 *
 * This interface extends {@link ElectrsDatabase} with write operations for maintaining
 * an electrs-compatible blockchain database. All write operations use RocksDB WriteBatch
 * for atomicity across multiple column families.
 *
 * Example usage:
 * <pre>{@code
 * Path dbPath = Path.of("/path/to/electrs/db");
 * try (WritableElectrsDatabase db = new RocksDbWritableElectrsDatabase(dbPath, networkParams)) {
 ...
 * }</pre>
 */
public interface WritableElectrsDatabase extends ElectrsDatabase {
